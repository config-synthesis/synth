...
```bash
elastic-file-system$ sam logs -n HelloEfsFunction --stack-name elastic-file-system --tail
```
...
```bash
elastic-file-system$ pip install pytest pytest-mock --user
elastic-file-system$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name elastic-file-system
```
