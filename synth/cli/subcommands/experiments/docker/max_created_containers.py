"""Synth max created containers experiment CLI."""
...
from synth.experiments.docker import max_created_containers
from synth.paths import EXPERIMENTS_OUTPUT_DIR


EXPERIMENT_OUTPUT_DIR = EXPERIMENTS_OUTPUT_DIR / 'max_created_containers'
...
def init_subparser(subparsers: SubparsersAction):
    ...
    parser = subparsers.add_parser(
        'max-created-containers',
        help='Run the Synth max created containers experiment.',
        description='Run the Synth max created containers experiment. This '
                    'experiment continues to create Docker containers for '
                    'a set of images until an error is encountered. It '
                    'records how many containers could be created based on '
                    'each image.',
    )
...
def run(args: Namespace):
    """Run the Docker max created containers experiment.
    ...
    # Run experiment.
    df, exceptions = max_created_containers.run()
    ...
    with pandas.option_context('display.max_rows', None,
        ...
        print('Container Creation Times By Image')
        print(
            df[['image', 'creation_time']]
            .groupby('image')
            .describe()
        )
        print()
