...
def join(cmd_list: Sequence[str]) -> str:
    """Join a command list into a command string.
...
def quote(part: str) -> str:
    ...
    if not part:
        ...
        return part
    elif any(char in part for char in SHELL_SPECIAL_CHARS) or '\n' in part:
        part = part.replace("'", "'\\''")
        part = part.replace('\n', '\\n')
        return f"$'{part}'"
    else:
